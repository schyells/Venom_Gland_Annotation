...
parser = argparse.ArgumentParser(description='''Automated annotation of non toxin transcripts transcripts.
Version July 14 2023.
Will return a folder of all of the .seq files containing the non-toxins requires the input of from Busco.py''')
...
parser.add_argument("-b","--busco",
                    type=str,
                    help="Text file of the busco ortholog information database")
...
parser.add_argument("-a","--animal",
                    ...
                    type=str,
                    default="Animal",
                    help="Animal Name and number ex Ssexp-C0184")
													...
													if args.sig == "Y" or args.sig =="y":
														...
														for alignment in rec.alignments:
															if aligncount == 1:
																...
																gene= gene.replace(')','')
																gene= gene.replace("'","")
																translation1 = prot3[0:43]
																translation2 = textwrap.wrap(prot3[43:],58)
																n=open(args.animal+'_'+gene+'_'+seqn.replace('_','-')+'.seq','w+')
																n.write('LOCUS       '+args.animal+'_'+gene+'_'+seqn.replace('_','-')+'        '+str(len(str(seq_record.seq)))+' bp    DNA     linear       '+now.strftime("%d-%b-%y")+'\n'+seqn+'\n'+'FEATURES             Location/Qualifiers'+'\n'+'     CDS             '+str(start+b)+'..'+str((len(prot3)*3)+start+b-1)+'\n'+'                     /note="'+gene+'"\n'+'                     /translation="'+translation1 +'\n                     '+'\n                     '.join(translation2)+'\n     source          1..'+str(len(str(seq_record.seq)))+'\n                     /dnas_title="'+args.animal+'_'+gene+'_'+seqn.replace('_','-')+'"'+'\n^^\n'+str(seq))
																n.close()
																fin2 = 'mv *.seq '+args.output
																os.system(fin2)
